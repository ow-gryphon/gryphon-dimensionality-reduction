...
from .dim_reduct_utilities import standardize_variables, export_plot, pd_save_as_xlsx
...
def run_variable_cluster(
    ...
    if generate_charts:
        ...
        export_plot(figure, output_path=output_path, prefix="dendrogram")

    # saves results to xlsx
    if save_results_to_excel:
        save_name = f"linkage_matrix_{timestamp}"
        pd_save_as_xlsx(pd.DataFrame(z), output_path, save_name, sheet_name='linkage')
